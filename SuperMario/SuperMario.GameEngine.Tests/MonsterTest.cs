...
namespace SuperMario.GameEngine.Tests
{
    ...
    public class MonsterTest
    {
        ...
        public void MonsterMovingTest()
        {
            ...
            gameGround[4, 4] = ' ';
            #endregion
            list[0].CurrentDirection = "Right";
            monster.MonsterMoving(gameGround);
            Assert.AreEqual(list[0].X, 2);
            list[0].CurrentDirection = "Left";
            monster.MonsterMoving(gameGround);
            Assert.AreEqual(list[0].X, 1);
        }
        [TestMethod]
        public void SecondMonsterMovingTest()
        {
            Monster monster = new Monster(2, 2);
            List<Monster> list = new List<Monster>();
            list.Add(new Monster(3, 1));
